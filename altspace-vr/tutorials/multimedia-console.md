...
1. 首先，您需要确保您的内容已联机托管，因为您需要 URL。  (可以将照片上传到 altvr.com，将视频联机 .mp4 文件，或者使用 Dlive 实时流链接： https://dlive.tv/yourlivestream) 
...
* **提升的用户** -在最初配置多媒体控制台的空间中具有仲裁者或主机角色的用户
* **所有用户** -所有用户

这些角色堆栈的意义在于，此列表中所选的所有角色都将被授予使用该功能的权限。  示例： **提升的用户** 包括 **所有者** ，即使他们不是 AltspaceVR 中的仲裁者或主机 * *。 由角色分配控制的功能如下所示
...
> [!NOTE]
> 必需：放入与视频长度匹配的时间，以便能够正确转发视频。 例如，如果视频的时间为5分钟，长达300秒，则视频不会跳到下一段内容。
...
媒体列表是 JSON 结构的根的属性集，如角色和播放设置。  它是一个简单的数组，它可以包含以下媒体配置结构之一。  (参见上面的属性说明了解每个操作的详细信息。 ) 
